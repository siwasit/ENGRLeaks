...
export default function CourseTable() {
    ...
    const handleLessonDelete = async (lesson_id: string) => {
        ...
        try {
            await axios.delete(`https://engrleaks-backend.onrender.com/delete_lessons/${lesson_id}`, {
                headers: {
    ...
    const retrieveLessonByCourseId = async (course_id: string) => {
        try {
            const res = await axios.get(`https://engrleaks-backend.onrender.com/lessons/course/${course_id}`);
            if (res.status === 200) {
